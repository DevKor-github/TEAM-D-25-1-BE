...
const mockUser: User = {
    id: mockUserId, email: 'test@test.com', username: 'testuser', nickname: '테스트유저',
    password: 'hashedpassword', socialProvider: null, socialId: null, isPrivate: false,
    status: 'ACTIVE', createdAt: new Date(), updatedAt: new Date(),
    firebaseUid: 'firebase-uid-1', isOnboarded: true, profileImageUrl: null, lastWatered: null,
};

const mockSavedRestaurant: SavedRestaurant = {
    userId: mockUserId, restaurantId: mockRestaurantId, description: '버크셔K 특로스시켜야함',
    review: '맛있어요~', createdAt: new Date(), updatedAt: new Date(),
    recommendedByUsers: [], treeType: 2,
};
...
describe('TreeService (unit)', () => {
  ...
  describe('getTreesByLocation', () => {
    it('should call repository.getTreesByLocation and return its result', async () => {
      ...
      (repository.getTreesByLocation as jest.Mock).mockResolvedValue(expectedRes)

      const res = await service.getTreesByLocation(mockUserId, zoom, location)
      
      expect(repository.getTreesByLocation).toHaveBeenCalledWith(mockUserId, zoom, location)
  ...
  describe('waterTree', () => {
    it('lastWatered가 null이면 repository.waterTree를 호출해야 함', async () => {
      ...
      expect(repository.waterTree).toHaveBeenCalledWith(mockRestaurantId, mockUserId);
    });
    ...
    it('물을 줄 수 있는 경우 repository.waterTree를 호출하고 결과를 반환해야 함', async () => {
      ...
      const expectedResult = { ...mockSavedRestaurant, recommendedByUsers: [mockUserId] };
      (prisma.user.findUnique as jest.Mock).mockResolvedValue({ lastWatered });
  ...
  describe('plantTree', () => {
    ...
    it('중복된 태그가 있을 경우 BadRequestException을 던져야 함', async () => {
        ...
        await expect(service.plantTree(dtoWithDuplicateTags, mockUserId)).rejects.toThrow(
    ...
    it('모든 검증을 통과하면 repository.plantTree를 호출하고 결과를 반환해야 함', async () => {
        ...
        (prisma.restaurant.findUnique as jest.Mock).mockResolvedValue(mockRestaurant);
        (prisma.tag.count as jest.Mock).mockResolvedValue(plantTreeDto.tagIds.length);
        ...
        expect(repository.plantTree).toHaveBeenCalledWith(plantTreeDto, mockUserId);
        expect(result).toEqual(expectedResult);
    });
  ...
  describe('getRecommendations', () => {
    it('should call repository.getRecommendations', async () => {
      ...
      (repository.getRecommendations as jest.Mock).mockResolvedValue(expectedResult);
  ...
  describe('getFollowersTree', () => {
    it('should call repository.getFollowersTree with userId and restaurantId', async () => {
      ...
      (repository.getFollowersTree as jest.Mock).mockResolvedValue(expectedResult);

      const result = await service.getFollowersTree(mockUserId, mockRestaurantId);

      expect(repository.getFollowersTree).toHaveBeenCalledWith(mockUserId, mockRestaurantId);
      expect(result).toEqual(expectedResult);
    });
  });
});
